  ...
  then
    ...
    then
      ...
      then
        ...
        if [[ $BIKE_ID_TO_RETURN = 0 ]]
        then
          RENTAL_ID=$($PSQL "SELECT rental_id, bike_id FROM rentals INNER JOIN customers USING(customer_id) WHERE phone = '$PHONE_NUMBER' AND date_returned IS NULL ORDER BY rental_id")
          echo "$RENTAL_ID" | while read ID BAR B_ID
          do
            # update date_returned
            RETURN_BIKE_RESULT=$($PSQL "UPDATE rentals SET date_returned = NOW() WHERE rental_id = $ID")
            # set bike availability to true
            SET_TO_TRUE_RESULT=$($PSQL "UPDATE bikes SET available = true WHERE bike_id = $B_ID")
          done
          # send to main menu
          MAIN_MENU "Thank you for returning your bikes."
        else
